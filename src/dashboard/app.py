...
def _demo_state():
    ...
    loss_D = (
        0.5 * np.exp(-2 * t) + 0.03 + 0.05 * rng.standard_normal(epochs) * 0.1
    ).tolist()
    evasion = np.clip(
        0.9 * np.exp(-4 * t) + 0.05 + 0.02 * rng.standard_normal(epochs), 0, 1
    ).tolist()
    return {
...
app.layout = dbc.Container(
    fluid=True,
    style={"background": BG, "minHeight": "100vh", "padding": "24px"},
    children=[
            ...
            [
                dbc.Col(
                    [
                        ...
                        html.P(
                            "Adversarial ML · Generative Attack Simulation · Live Threat Monitor",
                            style={
                                "color": "#8b949e",
                                "marginTop": "4px",
                                "fontSize": "13px",
                ...
                dbc.Col(
                    dbc.Card(
                        [
                            ...
                            dbc.CardBody(
                                dcc.Graph(
                                    id="gan-chart", config={"displayModeBar": False}
                                )
                            ),
                        ],
                        style={
                ...
                dbc.Col(
                    dbc.Card(
                        [
                            ...
                            dbc.CardBody(
                                dcc.Graph(
                                    id="metrics-bar", config={"displayModeBar": False}
                                )
                            ),
                        ],
                        style={
                ...
                dbc.Col(
                    dbc.Card(
                        [
                            ...
                            dbc.CardBody(
                                dcc.Graph(
                                    id="attack-pie", config={"displayModeBar": False}
                                )
                            ),
                        ],
                        style={
...
def refresh(_):
    ...
    mvals = [
        metrics.get(k, 0)
        for k in ["f1_weighted", "roc_auc", "accuracy", "precision", "recall"]
    ]
    ...
    if counts:
        ...
        fig_pie.update_layout(
            annotations=[
                dict(
                    text=f"<b>{total_att:,}</b><br><span style='font-size:11px'>attacks</span>",
                    x=0.5,
                    y=0.5,
                    showarrow=False,
